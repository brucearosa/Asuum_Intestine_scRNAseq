...
# 10. find differentially expressed genes. I tried wilcox, MAST and DESeq2 and also found their intersections. But finally decided to only use MAST. Also output cluster averages.
...
s1.t<-CreateSeuratObject(counts = s1d,project="S1",min.cells = 3, min.features = 100)
s3.t<-CreateSeuratObject(counts = s3d,project="S3",min.cells = 3, min.features = 100)
s4.t<-CreateSeuratObject(counts = s4d,project="S4",min.cells = 3, min.features = 100)
rm(s1d,s3d,s4d)
...
s1.t[["percent.mt"]] <- PercentageFeatureSet(s1.t, features=mtgenes)
s1.t<-subset(s1.t,subset = percent.mt < 1)
s3.t<-CreateSeuratObject(counts = s3d,project="S3",min.cells = 3, min.features = 100)
mtgenes=c("AgB02-g498","AgE51-g006","AgE51-g007","AgR009X-g325","AgR009X-g327","AgR009X-g326","AgR022-g083")
mtgenes=paste("gene:",mtgenes,sep="")
s3.t[["percent.mt"]] <- PercentageFeatureSet(s3.t, features=mtgenes)
s3.t<-subset(s3.t,subset = percent.mt < 1)
s4.t<-CreateSeuratObject(counts = s4d,project="S4",min.cells = 3, min.features = 100)
mtgenes=c("AgB02-g498","AgE51-g006","AgE51-g007","AgR009X-g325","AgR009X-g327","AgR009X-g326","AgR022-g083")
mtgenes=paste("gene:",mtgenes,sep="")
s4.t[["percent.mt"]] <- PercentageFeatureSet(s4.t, features=mtgenes)
s4.t<-subset(s4.t,subset = percent.mt < 1)
# Normalize using SCTransform
for(i in c("s1.t","s3.t","s4.t")){assign(i,SCTransform(get(i),verbose=FALSE))}
# Save out RDS files if desired
saveRDS(s1.t, file = "S1.rds")
saveRDS(s3.t, file = "S3.rds")
saveRDS(s4.t, file = "S4.rds")

# Step 2. find anchors and integrate them into s134 intergated Seurat object
int.features<-SelectIntegrationFeatures(object.list = c(s1.t,s3.t,s4.t), nFeatures=2000)
int.list <- PrepSCTIntegration(object.list = c(s1.t,s3.t,s4.t),anchor.features = int.features)
...
dev.off()
# Find Doublets
head(s134@meta.data,n=2)
annotations<-s134@meta.data$integrated_snn_res.0.4
homotypic.prop <- modelHomotypic(annotations)
length(s134@meta.data$integrated_snn_res.0.4)
...
# Step 4. Make new S1,S3,S4 objects (as in step 1 above). filter the doublets found in step 3 above from the corresponding sample object.
s1d<-Read10X(data.dir="<path_to_S1_filtered_feature_bc_matrix>")
s3d<-Read10X(data.dir="<path_to_S3_filtered_feature_bc_matrix>")
s4d<-Read10X(data.dir="<path_to_S4_filtered_feature_bc_matrix>")
...
# Remove cells with high occurence of mitochondrial genes (for these rebuilt Seurat objects)
s1<-CreateSeuratObject(counts = s1d,project="S1",min.cells = 3, min.features = 100)
mtgenes=c("AgB02-g498","AgE51-g006","AgE51-g007","AgR009X-g325","AgR009X-g327","AgR009X-g326","AgR022-g083")
mtgenes=paste("gene:",mtgenes,sep="")
